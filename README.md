# Hi, I'm Adithiya S.B 👋

### AI & Machine Learning Student | Full Stack Developer

*Building practical projects to grow into an industry-ready AI engineer.*

[LinkedIn](https://www.linkedin.com/in/adithiya-s-b-786b20290) | 
[Instagram](https://www.instagram.com/adithiya.mac?igsh=cWQ4MzQ2aGV5bHpn) | 
[Email](mailto:adithiyasb34@gmail.com)

---

## 🚀 About Me

* 🎓 B.Tech student in **Artificial Intelligence & Machine Learning** (SRM Trichy)
* 💡 Believe in **learning by building real-world projects**
* 🧠 Interested in **AI, Machine Learning, Computer Vision, and Backend Systems**
* 🎯 Goal: Secure **internships, placements, and research opportunities** while strengthening core fundamentals

---

## 💻 Tech Stack

| Category              | Tools & Technologies                    |
| :-------------------- | :-------------------------------------- |
| **Languages**         | Python, Java, JavaScript                |
| **AI & Data Science** | TensorFlow, PyTorch, OpenCV, Pandas     |
| **Web Development**   | React, Three.js, Tailwind CSS           |
| **Backend & Cloud**   | Flask, Spring Boot, MySQL, MongoDB, GCP |

---

## 🌟 Featured Projects

| Project                     | Description                                                                                          | Tech Stack                 |
| :-------------------------- | :--------------------------------------------------------------------------------------------------- | :------------------------- |
| **SmartGrocer-AI**          | ML-based system to predict food expiry and enable location-aware local food sharing to reduce waste. | Python, ML, Flask, MongoDB |
| **DSA-Grocer**              | Data-structures-based grocery sharing platform focused on efficiency and scalability.                | Java, DSA                  |
| **Object Detection System** | Real-time computer vision system capable of detecting multiple object classes.                       | Python, OpenCV, YOLO       |
| **Green Roof Maintenance**  | Autonomous monitoring system using IoT sensors and ML for urban green roofs.                         | IoT, ML, Sensors           |

---

## 🛠️ What I’m Currently Working On

* Strengthening **Machine Learning & Deep Learning fundamentals**
* Building **project-based applications** using Python & Java
* Improving **DSA and problem-solving skills**
* Learning **clean code practices and backend system design**

---

## 📊 GitHub Stats

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Adithiya-SB&show_icons=true&theme=dark)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Adithiya-SB&layout=compact&theme=dark)


---

## 🤝 Let’s Connect

I’m open to:

* 🤖 AI & ML student projects
* 🔬 Research collaborations
* 🧑‍💻 Hackathons and internships

📫 Feel free to connect if you’d like to collaborate or discuss technology.

⭐ *If you find my projects useful, consider starring my repositories!*
